"""
Dual-arm demonstration using VLA model: Build the word "VLA".

This script uses the trained GF-VLA model to control dual-arm robots
to arrange Jenga blocks to form the letters "VLA" on a table.
"""
...
def main():
    """Main function to run VLA-based letter building."""
    parser = argparse.ArgumentParser(
        description="Dual-arm VLA demonstration: Build the word 'VLA' with Jenga blocks"
    )
    
    # Model arguments (same as demo_vla_tower.py)
    parser.add_argument('--model-path', type=str, required=True, help='Path to trained VLA model checkpoint')
    ...
    parser.add_argument('--predict-mode', type=str, default='diff+ar', choices=['diff', 'ar', 'diff+ar'])
    ...
    parser.add_argument('--load-pointcloud', type=int, default=1)
    parser.add_argument('--use-robot-state', type=int, default=1)
    parser.add_argument('--slow-fast-ratio', type=int, default=4)
    parser.add_argument('--training-diffusion-steps', type=int, default=100)
    parser.add_argument('--llm-middle-layer', type=int, default=32)
    parser.add_argument('--action-chunk', type=int, default=1)
    parser.add_argument('--lang-subgoals-exist', type=int, default=0)
    
    # Hardware arguments
    parser.add_argument('--ur5e-ip', type=str, default='192.168.1.101')
    parser.add_argument('--ur10e-ip', type=str, default='192.168.1.102')
    parser.add_argument('--robotiq-port', type=str, default='/dev/ttyUSB0')
    parser.add_argument('--barrett-ip', type=str, default='192.168.1.103')
    parser.add_argument('--camera-type', type=str, default='realsense', choices=['realsense', 'kinect', 'simulation'])
    
    # Task arguments
    parser.add_argument(
        '--instruction',
        type=str,
        default='Arrange Jenga blocks on the table to form the letters V, L, and A',
