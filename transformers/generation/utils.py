...
class GenerationMixin:
    ...
    def _extract_slow_past_from_model_output(self, outputs: ModelOutput, standardize_cache_format: bool = False):
        slow_past_key_values = outputs.slow_past_key_values

        # Bloom fix: standardizes the cache format when requested
        if standardize_cache_format and hasattr(self, "_convert_to_standard_cache"):
            batch_size = outputs.logits.shape[0]
            slow_past_key_values = self._convert_to_standard_cache(slow_past_key_values, batch_size=batch_size)
            
        return slow_past_key_values
    ...
    def _update_model_kwargs_for_generation(
        ...
        model_kwargs["slow_past_key_values"] = self._extract_slow_past_from_model_output(
            outputs, standardize_cache_format=standardize_cache_format
        )
