...
# === GF-VLA (Graph-Fused Vision-Language-Action) Configs ===

@dataclass
class Exp_GFVLA_DualArm_BlockAssembly(VLAConfig):
    """
    GF-VLA configuration for dual-arm block assembly tasks.
    Based on "Graph-Fused Vision-Language-Action for Policy Reasoning in Multi-Arm Robotic Manipulation"
    """
    vla_id: str = "gfvla-dualarm-blockassembly"
    base_vlm: Union[str, Path] = "prism-dinosiglip-224px+7b"
    ...
    # Data Mixture Parameters
    data_mix: str = "oxe_magic_soup_plus_minus"  # Can be customized for dual-arm demonstrations
    shuffle_buffer_size: int = 10_000
    ...
    # GF-VLA specific parameters
    graph_embed_dim: int = 512
    enable_cot: bool = True  # Chain-of-Thought prompting
    enable_verification: bool = True  # Self-verification
    enable_cross_arm: bool = True  # Cross-arm allocation
    temporal_window_size: int = 10  # For information-theoretic graph extraction
    entropy_threshold: float = 0.5  # For active region detection
...
class VLARegistry(Enum):
    ...
    # === GF-VLA Configs ===
    EXP_GFVLA_DUALARM_BLOCKASSEMBLY = Exp_GFVLA_DualArm_BlockAssembly
