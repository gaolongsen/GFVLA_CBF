...
class DinoSigLIPViTBackbone(VisionBackbone):
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor], pc_forward=False) -> torch.Tensor:
        """Runs the transformed image/pixel tensors through each vision backbone, returning concatenated patches."""
        dino_patches = self.dino_featurizer(pixel_values["dino"], pc_forward=pc_forward)
        siglip_patches = self.siglip_featurizer(pixel_values["siglip"], pc_forward=pc_forward)
    ...
    @property
    def embed_dim_dino(self) -> int:
        return self.dino_featurizer.embed_dim

    @property
    def embed_dim_siglip(self) -> int:
